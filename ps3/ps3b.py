...
class TreatedPatient(Patient):
    ...
    def update(self):
        ...
        for j in viruses_copy_2:
            try:
                self.viruses.append(j.reproduce(popDensity, list(self.activeDrugs)))
            except NoChildException:
                continue
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    for i in range(numTrials):
        ...
        for k in range(150):
            ...
            resistVirusCount.append(patient.getResistPop(['guttagonol']))
        data = data + virusCount
        data1 = data1 + resistVirusCount
    data_avg = data/numTrials
    data1_avg = data1/numTrials

    pylab.plot(list([float('{0:.1f}'.format(i)) for i in data_avg]), label = "Total")   
    pylab.plot(list([float('{0:.1f}'.format(j)) for j in data1_avg]), label = "Guttagonol-resistant")
